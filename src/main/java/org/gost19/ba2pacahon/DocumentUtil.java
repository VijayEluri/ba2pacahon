...
import magnetico.ws.document.AccessDeniedException_Exception;
import magnetico.ws.document.BigArchiveServerException_Exception;
import magnetico.ws.document.DocumentEndpoint;
import magnetico.ws.document.DocumentService;
import magnetico.ws.document.DocumentType;
import magnetico.ws.document.DocumentTemplateType;
import magnetico.ws.document.JaxbObjectType;

public class DocumentUtil
{

	public static final QName DOCUMENT_QNAME = new QName("http://documents.bigarchive.magnetosoft.ru/", "DocumentService");
	...
	public List<String> listDocuments(String url, String ticket) throws MalformedURLException, AccessDeniedException_Exception,
			BigArchiveServerException_Exception
	{
	...
	private List<String> listDocs(String url, String ticket, JaxbObjectType jot) throws MalformedURLException,
			AccessDeniedException_Exception, BigArchiveServerException_Exception
	{
		...
		while (from < docsCount)
		{
			...
			for (String document : documents)
			{
				...
				// String ss = String.format("%s:%s:%s", document.substring(i1 +
				// 4, i2), document.substring(i3 + 10, i4),
				// document.substring(i5 + 8, i6));
				...
				// DocumentType dt = documentService.getDocument(ticket,
				// document);
	...
	public List<DocumentTemplateType> listDocumentTypes(String url, String ticket) throws MalformedURLException,
			AccessDeniedException_Exception, BigArchiveServerException_Exception
	{
	...
	private List<DocumentTemplateType> listTemplates(String url, String ticket, JaxbObjectType jot) throws MalformedURLException,
			AccessDeniedException_Exception, BigArchiveServerException_Exception
	{
		service = new DocumentService(new URL(url), DOCUMENT_QNAME);
		documentService = service.getDocumentServiceEndpointPort();
