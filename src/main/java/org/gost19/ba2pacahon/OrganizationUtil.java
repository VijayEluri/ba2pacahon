...
public class OrganizationUtil
{
	...
	public OrganizationUtil(String url, String namespace, String name)
	{
		try
		{
			organizationInvoker = new OrganizationEntityService(new URL(url), new QName(namespace, name))
	...
	public List<Department> getDepartments()
	{
		...
		if (organizationInvoker != null)
		{
			try
			{
				...
				EntityContainerType ec = organizationInvoker.queryEntityContainer("department", query);
	...
	public List<EntityType> getUsers()
	{
		...
		try
		{
			...
			result = organizationInvoker.queryEntityContainer("contact", query).getEntities().getEntityList();
	...
	protected PreparedQueryType createQuery(String tokens, String mainClause, String valuePrefix, String valueSuffix)
	{
	...
	public Department getDepartmentByUserId(String userId, String locale)
	{
		...
		try
		{
			List<String> deptIds = organizationInvoker.getAdjacency(userId, USER_DEPARTMENT_RELATION,
					AdjacencyTypeEnum.FORWARD);
	...
	public List<Department> getDepartmentsByParentId(String parentId, String locale)
	{
		...
		try
		{
			...
			List<String> uids = organizationInvoker.queryUids("department", query);
			if (!uids.isEmpty())
			{
				List<EntityType> entities = organizationInvoker.getEntities(uids);
